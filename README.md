...
`tests/unit/test_registry_coverage.py` падает, если в реестре есть эндпоинт без соответствующего
...
`scripts/live_smoke.sh` гоняет **все 28 эндпоинтов** через настоящий `yac` на боевом Audience API
(в отличие от read-only `tests/e2e/test_live_api.py`). ⚠️ Скрипт **мутирует боевой аккаунт**: создаёт
и удаляет реальные объекты (с префиксом `SMOKE yac`), но самоочищается через `trap EXIT` — работает
только на своих данных, чужие сегменты не трогает. В CI не запускается (нужен реальный токен и квота
записи). Запуск: токен в `.env`, затем `bash scripts/live_smoke.sh`. Подробнее —
[Разработка](https://axisrow.github.io/yandex-audience-cli/development/).
